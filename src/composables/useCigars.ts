...
export const useCigars = () => {
  ...
  const addCigar = async (cigarData: Omit<CigarEntry, 'id' | 'userId' | 'smokedAt'>) => {
    if (!user.value) {
    ...
    try {
      ...
      return onSnapshot(qWithOrder, (snapshot) => {
        ...
        return onSnapshot(qFallback, (snapshot) => {
          console.log('Fallback query - Firestore snapshot received:', snapshot.size, 'documents')
          cigars.value = snapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data()
          })) as CigarEntry[]
  ...
  const stats = computed(() => {
    ...
    const todayCigars = cigars.value.filter(c => {
      const cigarDate = c.smokedAt?.toDate ? c.smokedAt.toDate() : new Date(c.smokedAt)
